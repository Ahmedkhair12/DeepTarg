...
from torch_geometric.nn import GraphSAGE, to_hetero                                              ## MESSAGE PSSING
import torch
from torch import Tensor
from torch_geometric.data import HeteroData
...
class GNN(torch.nn.Module):
    def __init__(self, hidden_channels):
        super().__init__()
        self.conv1 = GraphSAGE(hidden_channels, hidden_channels)                                 ## NUMBER OF LAYERS
...
model = DeepTarg(data, hidden_channels=64)                                                     ## NUMBER OF CHANNELS
...
x_dict = {}
for epoch in range(1, 25):                                                                      ## EPOCHS
...
np.save("cross_comparisons/predictions_1.npy", preds)
np.save("cross_comparisons/ground_truths.npy", ground_truths)
